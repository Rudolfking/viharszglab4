...
public abstract class Vehicle extends NamedObject {
    ...
    protected Cell chooseFrom(Cell[] cells) {
		...
		/*// vÃ¡lasztÃ¡s a preferÃ¡lt cella ismeretÃ©ben
		boolean prefCellIsReachable = false;
		// preferÃ¡lt cella elÃ©rhetÅ‘sÃ©gÃ©nek ellenÅ‘rzÃ©se (Ãºton nem)
		if (cell.getRoad() == null) {
			for (Cell c : cells)
				if(c == preferredCell) prefCellIsReachable = true;			
		}
		// ha elÃ©rhetÅ‘, akkor azt vÃ¡lasztjuk
		if(prefCellIsReachable)
			return preferredCell;
		else {
			// ha nem, vÃ©letlenszerÅ±en vÃ¡lasztunk			
			Random r = new Random();			
